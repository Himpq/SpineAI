...
Future<void> loadSavedPrefs(ProviderContainer container) async {
  ...
  if (savedTheme != null && savedTheme >= 0 && savedTheme < ThemeMode.values.length) {
    container.read(themeModeProvider.notifier).state = ThemeMode.values[savedTheme];
  }
}
...
final patientRepoProvider = Provider((ref) => PatientRepository(ref.read(apiClientProvider)));
final reviewRepoProvider = Provider((ref) => ReviewRepository(ref.read(apiClientProvider)));
final chatRepoProvider = Provider((ref) => ChatRepository(ref.read(apiClientProvider)));
final portalRepoProvider = Provider((ref) => PortalRepository(ref.read(apiClientProvider)));
final overviewRepoProvider = Provider((ref) => OverviewRepository(ref.read(apiClientProvider)));
final questionnaireRepoProvider = Provider((ref) => QuestionnaireRepository(ref.read(apiClientProvider)));
...
class AuthState {
  ...
  AuthState copyWith({AuthMode? mode, UserModel? user, String? portalToken, Map<String, dynamic>? portalData, bool? loading}) {
    return AuthState(
      mode: mode ?? this.mode,
      user: user ?? this.user,
...
class AuthNotifier extends StateNotifier<AuthState> {
  ...
  Future<void> _checkSession() async {
    try {
      ...
      if (savedToken != null && savedToken.isNotEmpty) {
        try {
          final data = await ref.read(portalRepoProvider).getPortalData(savedToken);
          state = AuthState(mode: AuthMode.patient, portalToken: savedToken, portalData: data, loading: false);
  ...
  Future<void> enterPortal(String token) async {
    ...
    final patientName = (data['patient'] as Map<String, dynamic>?)?['name'] as String? ?? '患者';
    await _saveToHistory(token, patientName);
    state = AuthState(mode: AuthMode.patient, portalToken: token, portalData: data, loading: false);
    final ws = ref.read(wsClientProvider);
    final url = ref.read(serverUrlProvider);
    ws.connect(url, kind: 'patient', name: data['patient']?['name'] ?? '患者');
    ws.subscribe('system');
  ...
  void _onToast(Map<String, dynamic> msg) {
    ...
    ref.read(toastStreamProvider.notifier).state = ToastMessage(title: title, message: message, level: level);
  }

  void _onFeedNew(Map<String, dynamic> _) {
    ref.invalidate(overviewProvider);
  }
...
final authChangeNotifierProvider = Provider<AuthChangeNotifier>((ref) => AuthChangeNotifier(ref));

// ── Patient List ──
final patientListProvider = FutureProvider.autoDispose<List<PatientModel>>((ref) async {
  return ref.read(patientRepoProvider).getPatients();
});

// ── Review List ──
final reviewListProvider = FutureProvider.autoDispose<List<ExamModel>>((ref) async {
...
// ── Questionnaire List ──
final questionnaireListProvider = FutureProvider.autoDispose<List<Map<String, dynamic>>>((ref) async {
...
final chatUnreadCountProvider = Provider<int>((ref) {
  ...
  return ov.whenOrNull(data: (d) => (d['stats'] as Map?)?['unread_messages'] as int? ?? 0) ?? 0;
});

final pendingReviewCountProvider = Provider<int>((ref) {
  final ov = ref.watch(overviewProvider);
  return ov.whenOrNull(data: (d) => (d['stats'] as Map?)?['pending_reviews'] as int? ?? 0) ?? 0;
});
...
class ToastMessage {
  ...
  final String level;
  ToastMessage({required this.title, required this.message, this.level = 'info'});
}
