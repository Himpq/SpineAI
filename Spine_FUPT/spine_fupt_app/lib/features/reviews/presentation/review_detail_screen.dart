...
class _ReviewDetailScreenState extends ConsumerState<ReviewDetailScreen> {
  ...
  String? _error;
  String? _commentChannel;

  @override
  void initState() {
    super.initState();
    final ws = ref.read(wsClientProvider);
    ws.on('case_comment', _handleCaseComment);
    ws.on('feed_new', _handleFeedNew);
    _load();
  }

  void _syncCommentChannel(String? channel) {
    if (_commentChannel == channel) return;
    final ws = ref.read(wsClientProvider);
    if (_commentChannel != null) {
      ws.unsubscribe(_commentChannel!);
    }
    _commentChannel = channel;
    if (channel != null) {
      ws.subscribe(channel);
    }
  }

  void _handleCaseComment(Map<String, dynamic> payload) {
    if (!mounted || _commentChannel == null) return;
  ...
  void _handleFeedNew(Map<String, dynamic> payload) {
    final item = payload['item'] as Map<String, dynamic>? ?? {};
    final eventType = item['event_type'] as String? ?? '';
    final examId = item['exam_id'] as int?;
    if (examId != widget.examId) return;
    if ({
      'xray_upload',
      'inference_result',
      'inference_failed',
  ...
  Future<void> _load() async {
    try {
      final data =
          await ref.read(reviewRepoProvider).getReviewDetail(widget.examId);
      if (!mounted) return;
      _syncCommentChannel(data.commentChannel);
      setState(() {
        _exam = data;
  ...
  void dispose() {
    final ws = ref.read(wsClientProvider);
    ws.off('case_comment', _handleCaseComment);
    ws.off('feed_new', _handleFeedNew);
    if (_commentChannel != null) {
      ws.unsubscribe(_commentChannel!);
    }
    _noteCtrl.dispose();
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => _showShareSheet(context, exam),
          ),
        ],
      ),
      body: Column(
        children: [
          ...
          // Info panel section
          Expanded(flex: 45, child: _buildInfoPanel(exam, theme)),
        ],
      ),
    );
  }
  ...
  Widget _buildInferenceTab(ExamModel exam, ThemeData theme) {
    return ListView(
      ...
      children: [
        ...
        if (exam.spineClassText != null) _InfoRow('分类', exam.spineClassText!),
        if (exam.spineClassConfidence != null)
          _InfoRow(
            '置信度',
            '${(exam.spineClassConfidence! * 100).toStringAsFixed(1)}%',
          ),
        ...
        if (exam.severityLabel != null) _InfoRow('严重程度', exam.severityLabel!),
        if (exam.improvementValue != null)
          _InfoRow(
            '改善值',
            '${exam.improvementValue! > 0 ? "+" : ""}${exam.improvementValue!.toStringAsFixed(1)}°',
          ),
  ...
  Widget _buildReviewTab(ExamModel exam, ThemeData theme) {
    final statusLabel = exam.isReviewed
        ? '已复核'
        : exam.isFailed
            ? '推理失败'
            : exam.isInferring
                ? '推理中'
                : '待复核';
    final statusColor = exam.isReviewed
    ...
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: 16),
        FilledButton.icon(
          onPressed: _submitting || exam.isInferring
              ? null
              : () => _submitReview(exam),
                              ...
                              children: [
                                Row(
                                  children: [
                                    ...
                                    const SizedBox(width: 8),
                                    Text(
                                      _fmtTime(c['created_at']?.toString()),
                                      style: theme.textTheme.bodySmall,
                                    ),
                                  ],
                                ),
  ...
  Future<void> _submitReview(ExamModel exam) async {
    ...
    try {
      await ref.read(reviewRepoProvider).submitReview(
            exam.id,
            status: 'reviewed',
            reviewNote: _noteCtrl.text,
          );
      if (mounted) {
        ref.invalidate(reviewListProvider);
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('复核已提交')));
        _load(); // Reload
      }
    } catch (e) {
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('提交失败: ${ApiClient.friendlyError(e)}')),
        );
    } finally {
  ...
  Future<void> _undoReview(ExamModel exam) async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('已撤销复核')));
        _load();
      }
    } catch (e) {
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('撤销失败: ${ApiClient.friendlyError(e)}')),
        );
    } finally {
      if (mounted) setState(() => _submitting = false);
    }
  ...
  Future<void> _sendComment() async {
    ...
    try {
      final result =
          await ref.read(reviewRepoProvider).addComment(widget.examId, text);
      _commentCtrl.clear();
      ...
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('发送失败: ${ApiClient.friendlyError(e)}')),
        );
    }
  }
  ...
  void _showShareSheet(BuildContext context, ExamModel exam) {
    ...
    showModalBottomSheet(
      context: context,
      builder: (_) =>
          _ShareSheet(exam: exam, ref: ref, parentMessenger: parentMessenger),
    );
  }
}
...
class _ShareSheetState extends State<_ShareSheet> {
        ...
        children: [
          ...
          ListTile(
            ...
            subtitle: _shareUrl != null
                ? Text(_shareUrl!, style: const TextStyle(fontSize: 12))
                : null,
            onTap: _generateShareLink,
            trailing: _loadingLink
                ? const SizedBox(
                    width: 24,
                    height: 24,
  ...
  Future<void> _generateShareLink() async {
    ...
    try {
      final result = await widget.ref
          ...
          .createShareLink(widget.exam.id);
      ...
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('生成失败: ${ApiClient.friendlyError(e)}')),
        );
    }
  }
  ...
  Future<void> _showShareToUser() async {
    try {
      final targetList =
          await widget.ref.read(reviewRepoProvider).getShareTargets();
      if (!mounted) return;
          ...
          itemBuilder: (ctx, i) {
            ...
            return ListTile(
              leading: CircleAvatar(
                child: Text((t['display_name'] ?? t['username'] ?? '?')[0]),
              ),
              ...
              onTap: () async {
                ...
                try {
                  final result = await widget.ref
                      .read(reviewRepoProvider)
                      .shareToUser(widget.exam.id, t['id'] as int);
                    ...
                    SnackBar(
                      ...
                      action: convId != null
                          ? SnackBarAction(
                              ...
                              onPressed: () {
                                router.push(
                                  '/doctor/chat/$convId?name=${Uri.encodeComponent(targetName)}',
                                );
                              },
                            )
                          : null,
                  ...
                  rootMessenger.showSnackBar(
                    SnackBar(
                      content: Text('分享失败: ${ApiClient.friendlyError(e)}'),
                    ),
                  );
                }
              },
      ...
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('加载失败: ${ApiClient.friendlyError(e)}')),
        );
    }
  }
}
