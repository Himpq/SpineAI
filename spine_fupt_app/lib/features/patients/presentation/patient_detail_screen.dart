...
final _patientDetailProvider = FutureProvider.autoDispose
...
class _PatientDetailScreenState extends ConsumerState<PatientDetailScreen> {
  ...
  void initState() {
    super.initState();
    ref.read(wsClientProvider).on('feed_new', _handleFeedNew);
  }
  ...
  void _handleFeedNew(Map<String, dynamic> payload) {
    ...
    final patientId = item['patient_id'] as int?;
    if (patientId != widget.patientId) return;
    if ({
      'xray_upload',
      'inference_result',
      'inference_failed',
  ...
  Future<void> _doRefresh() async {
    ...
    try {
      ref.invalidate(_patientDetailProvider(widget.patientId));
      await ref.read(_patientDetailProvider(widget.patientId).future);
      if (mounted) {
  ...
  @override
  void dispose() {
    ref.read(wsClientProvider).off('feed_new', _handleFeedNew);
    super.dispose();
  }
    ...
    return Scaffold(
      ...
      body: detail.when(
        ...
        data: (data) {
          ...
          final exams = (patientData['exams'] as List? ?? [])
              ...
              Wrap(
                ...
                children: [
                  _ActionChip(
                    ...
                    label: '上传影像',
                    onTap: () => _uploadImage(context, ref),
                  ),
                  _ActionChip(
                    icon: Icons.chat,
                    label: '发起聊天',
                    onTap: () => _startChat(context, ref, patient),
                  ),
                  _ActionChip(
                    icon: Icons.calendar_today,
                    label: '安排日程',
                    onTap: () => _addSchedule(context, ref),
                  ),
                  if (portalToken != null)
                    _ActionChip(
                      icon: Icons.qr_code,
                      label: '患者二维码',
                      onTap: () =>
                          _showPatientQr(context, portalToken, patient.name),
                    ),
                ],
              ),
              ...
              if (exams.isEmpty)
                const Card(
                  child: Padding(
                    ...
                    child: Center(child: Text('暂无检查记录')),
                  ),
                )
              else
                ...exams.map(
                  (exam) => _ExamTimelineItem(
                    exam: exam,
  ...
  Future<void> _uploadImage(BuildContext context, WidgetRef ref) async {
    ...
    try {
      await ref
          .read(patientRepoProvider)
          .uploadExam(widget.patientId, File(picked.path));
      ref.invalidate(_patientDetailProvider(widget.patientId));
      ref.invalidate(reviewListProvider);
      if (context.mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('上传成功，推理中...')));
      }
    } catch (e) {
      if (context.mounted) {
        ScaffoldMessenger.of(
  ...
  Future<void> _startChat(
    ...
    try {
      final conv = await ref
          ...
          .createConversation(type: 'patient', patientId: patient.id);
            ...
            FilledButton(
              onPressed: () async {
                try {
                  await ref
                      .read(patientRepoProvider)
                      .updatePatient(widget.patientId, {
                    ...
                    Row(
                      children: [
                        ...
                        if (exam.cobbAngle != null)
                          Text(
                            'Cobb: ${exam.cobbAngle!.toStringAsFixed(1)}°',
                            style: theme.textTheme.bodyMedium,
                          ),
                        if (exam.severityLabel != null) ...[
                          ...
                          Text(
                            exam.severityLabel!,
                            style: TextStyle(
                              color: _severityColor(exam.severityLabel!),
                              fontSize: 13,
                            ),
                          ),
                        ],
...
class _CobbTrendChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final values =
        trend.map((t) => (t['cobb_angle'] as num?)?.toDouble() ?? 0).toList();
    final maxVal =
        values.reduce((a, b) => a > b ? a : b).clamp(10, 90).toDouble();
        ...
        child: SizedBox(
          ...
          child: Row(
            ...
            children: List.generate(trend.length, (i) {
              final val = values[i];
              final barH = (val / maxVal * barAreaHeight).clamp(
                4.0,
                barAreaHeight,
              );
