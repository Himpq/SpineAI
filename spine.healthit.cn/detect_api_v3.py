...
def load_models():
    ...
    if _HR_MODEL is None:
        ...
        state = torch.load(HRNET_WEIGHTS, map_location=DEVICE, weights_only=False, pickle_module=pickle_module)
    ...
    if _HED_MODEL is None:
        ...
        state = torch.load(HED_WEIGHTS, map_location=DEVICE, weights_only=False, pickle_module=pickle_module)
        model.load_state_dict(state["model_state"] if isinstance(state, dict) and "model_state" in state else state, strict=False)
        model.eval()
        _HED_MODEL = model
    if _OPLL_MODEL is None:
        model = UNetOPLL(in_channels=1, out_channels=1).to(DEVICE)
        state = torch.load(OPLL_WEIGHTS, map_location=DEVICE, weights_only=False)
...
def infer_hrnet(img_bgr: np.ndarray) -> Tuple[List[Tuple[float, float]], np.ndarray]:
    ...
    return coords, hm.detach().cpu().numpy()
...
def infer_hed_line(img_bgr: np.ndarray) -> Tuple[int, np.ndarray]:
    ...
    _, hed, _ = load_models()
    with torch.no_grad():
        prob = hed(tensor)[-1][0, 0].cpu().numpy()
...
def infer_opll(img_bgr: np.ndarray) -> np.ndarray:
    ...
    with torch.no_grad():
        pred = opll(tensor)
    pred_mask = (pred.squeeze().cpu().numpy() * 255).astype(np.uint8)
    return pred_mask
